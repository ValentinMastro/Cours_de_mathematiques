...
\exemple{%
\begin{center}
    \begin{tabularx}{0.8\linewidth}{|l|X|X|X|}\hline
        & filles & garçons & total \\\hline
        effectifs & 11 & 14 & 25 \\\hline
        fréquences & $\dfrac{11}{25}$ & $\dfrac{14}{25}$ & \\\hline
    \end{tabularx}
\end{center}
    En pourcentages : 
    \begin{itemize}
        \item $\dfrac{11}{25} \times 100 = \qty{44}{\%}$
...
\begin{questions}
    ...
    \begin{center}
        \begin{tikzpicture}
            \draw[step=0.2,black!20!white,thin] (0,0) grid (7,5);
            \draw[->] (0,0) -- (7,0);
            \draw[->] (0,0) -- (0,5);
            \foreach \y in {1,2,3,4} {
                \draw (-0.2,\y) node[left]{\pgfmathparse{\y*5}\pgfmathresult} -- (0.2,\y);
            }
