...
local function perimetre_cercle()
    ...
    local enonce = scriptsize("Calculer le périmètre d'un cercle de rayon \\qty{" .. r .. " }{\\centi\\metre}")
...
local function perimetre_cercle_diametre()
    ...
    local enonce = scriptsize("Calculer le périmètre d'un cercle de diamètre \\qty{" .. d .. " }{\\centi\\metre}")
...
local function perimetre_carre()
    ...
    local reponses = map(scriptsize, {4*c, 3*c, 2*c, c, 5*c})
    afficher_question(enonce, reponses, 1)
end
...
return { qA = convertion_hh_mm_ss, qB = convertion_secondes, qC = convertion, qD = fractions_egales, qE = tableau_proportionnalite, qF = perimetre }