...
local function multiplier_diviser_puiss_10()
    ...
    then
        local enonce = "$" .. num(nombre) .. " \\times " .. num(facteur) .. " = $"
        local reponses = {num(nombre*facteur), num(nombre*facteur*10), num(nombre*facteur*100), num(nombre*facteur/10), num(nombre*facteur/100)}
        afficher_question(enonce, reponses, 1)
    else
        local enonce = "$" .. num(nombre) .. " \\div " .. num(facteur) .. " = $"
        local reponses = {num(nombre/facteur), num(nombre/facteur*10), num(nombre/facteur*100), num(nombre/facteur/10), num(nombre/facteur/100)}
        afficher_question(enonce, reponses, 1)
    end
end
...
local function poser_multiplication()
    ...
    local enonce = "$" .. num(a) .. " \\times " .. num(b) .. " =$"

    local reponses = {num(a*b), num(a*2*b), num(a*b/2), num(a*b*10), num(a*b*2/10)}
...
local function convertion()
    ...
    local reponses = map(small, map(num, {nombre * math.pow(10, indexes[1]), nombre * math.pow(10, indexes[2]), nombre * math.pow(10, indexes[3]), nombre * math.pow(10, indexes[4]), nombre * math.pow(10, indexes[5])}))

    if nombre==math.floor(nombre) then nombre = math.floor(nombre) end
    local enonce = "$\\qty{" .. nombre .. "}{" .. prefixes[1] .. unites[1] .. "} = ~~~\\unit{" .. prefixes[2] .. unites[1] .. "}$"

    afficher_question(enonce, reponses, 1)
end

return { qA = multiplier_diviser_puiss_10, qB = poser_multiplication, qC = convertion }