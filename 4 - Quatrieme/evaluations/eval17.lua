...
local function double_developpement()
    ...
    local reponses = map(scriptsize, {polyn(a*c, a*d+b*c, b*d), polyn(a+c, a+d+b+c, b+d), polyn(a*c, a*d+b*c-1, b*d), polyn(a*b, a*d+b*c-1, b*c), polyn(a+c, a+d+b+c+1, b+d)})
...
local function augmentation_pourcentage()
    local prix = math.random(10,49)
    local pourcentage = math.random(500,999) / 10

    local enonce = scriptsize("Un article est vendu " .. qty(a, "\\EURO") .. " puis son prix augmente de " .. qty(b, "\\percent") .. ". Son nouveau prix est :")

    local reponses = map(scriptsize, map(num, {num(a*(100+b)/100), num(a*(100-b)/100), num(b*(100+a)/100), num(b*(100-a)/100), num(a*b/100)}))
...
return { qA = pythagore, qB = calcul_thales, qC = factoriser, qD = double_developpement, qE = volume, qF = double_developpement_signes }