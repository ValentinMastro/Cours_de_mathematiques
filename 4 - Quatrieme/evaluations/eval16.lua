...
local function calcul_cote_angle_droit()
    ...
    local enonce = "\\begin{tikzpicture}[baseline=(current bounding box.center)] \\draw (0,0) -- (0.8,0) -- (0.8,0.8) -- cycle; \\draw[fill=black] (0.8,0) rectangle (0.7,0.1); \\node[above] at (0,0) {\\scriptsize{$" .. p1 .. "$}}; \\node[above right] at (0.8,0) {\\scriptsize{$" .. p2 .. "$}}; \\node[right] at (0.8,0.8) {\\scriptsize{$" .. p3 .. "$}}; \\node[anchor=west] at (1.3,0.7) {\\scriptsize{$" .. p3 .. p1 .. "=" .. arrondi(a,1) .. "$}}; \\node[anchor=west] at (1.3,0.4) {\\scriptsize{$" .. p1 .. p2 .. "=" .. arrondi(b,1) .. "$}}; \\node[anchor=west] at (1.3,0.1) {\\scriptsize{$" .. p2 .. p3 .. "=~?" .. "$}}; \\end{tikzpicture}"

    local reponses = map(scriptsize, {arrondi(c, 2), arrondi(math.sqrt(a*a+b*b), 2), arrondi(c + 0.5 + math.random(), 2), arrondi(c - 0.5 - math.random(), 2), arrondi(c - 0.5 - math.random(), 2) })
...
return { qA = moyenne_trouver_x, qB = ecriture_scientifique, qC = cosinus_angle, qD = resoudre_equation, qE = diviseurs, qF = augmentation_pourcentage, qG = diminution_pourcentage }